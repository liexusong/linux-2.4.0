...
void nf_debug_ip_local_deliver(struct sk_buff *skb)
{
	...
	if (!skb->dev) {
		...
		if (skb->nf_debug != ((1<<NF_IP_PRE_ROUTING)|(1<<NF_IP_LOCAL_IN))) {
...
{
	if (newskb->nf_debug != ((1 << NF_IP_LOCAL_OUT)|(1 << NF_IP_POST_ROUTING)))
	{
		printk("ip_dev_loopback_xmit: bad owned skb = %p: ", newskb);
...
void nf_debug_ip_finish_output2(struct sk_buff *skb)
{
	 ...
	 */
	if (skb->sk) {
		if (skb->nf_debug != ((1 << NF_IP_LOCAL_OUT)|(1 << NF_IP_POST_ROUTING)))
		{
...
nf_reinject(struct sk_buff *skb, struct nf_info *info, unsigned int verdict)
{
	...
	if (verdict == NF_ACCEPT) {
		verdict = nf_iterate(&nf_hooks[info->pf][info->hook], &skb, info->hook,
							 info->indev, info->outdev, &elem, info->okfn);
	}
	...
	case NF_QUEUE:
		nf_queue(skb, elem, info->pf, info->hook, info->indev, info->outdev,
				 info->okfn);
		break;
