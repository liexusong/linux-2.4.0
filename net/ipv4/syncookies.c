...
cookie_v4_check(struct sock *sk, struct sk_buff *skb, struct ip_options *opt)
{
	...
	if (ip_route_output(&rt,
						opt && opt->srr
							? opt->faddr : req->af.v4_req.rmt_addr,
						req->af.v4_req.loc_addr,
						sk->protinfo.af_inet.tos|RTO_CONN, 0))
	{
		tcp_openreq_free(req);
		return NULL;
	}
	...
	tcp_select_initial_window(tcp_full_space(sk),req->mss, &req->rcv_wnd,
							  &req->window_clamp, 0, &rcv_wscale);
